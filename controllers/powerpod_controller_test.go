...
func TestPowerPod_generateCPUScalingOpts(t *testing.T) {
	...
	opts := r.generateCPUScalingOpts(policy, cpuIDs)

	// Should only generate opts for CPUs 0 and 2, not 1
	require.Len(t, opts, 2)
	ids := []uint{opts[0].CPU.GetID(), opts[1].CPU.GetID()}
	assert.ElementsMatch(t, cpuIDs, ids)
	...
	for _, o := range opts {
		...
		assert.Equal(t, int(maxFreq), o.HWMaxFrequency)
		assert.Equal(t, int(minFreq), o.HWMinFrequency)
...
func TestPowerPod_Reconcile_MultipleDPDKContainersRejected(t *testing.T) {
	...
	// Only one CreateConnection and one AddCPUScaling call expected (for the first container).
	dpdkmk := new(DPDKTelemetryClientMock)
	dpdkmk.On("CreateConnection", &scaling.DPDKTelemetryConnectionData{
